...
#    <utt-id> <local-ark-indx> <global-ark-indx> <start-frame> <num-frames> <spk-id>
# For example:
#    100304-f-sre2006-kacg-A 1 2 4079 881 23

# If you're satisfied with the number of archives (e.g., 50-150 archives is
# reasonable) and with the number of examples per speaker (num2pdf) (e.g., 1000-5000
...
if [ $stage -le 6 ]; then
  ...
  # --frames-per-iter: target number of frames for each archive
  # --num-repeats: 保证在 allocate_egs.py 第 258 行中，spk 不会被用尽，每个 archive 产生足够数量的 chunks
  #   并且每一个 archive 中，所有 chunks 的长度都相等，长度在 --min-frames-per-chunk 与 --max-frames-per-chunk
  #   之间
  # chunk 就是从一个 utt 截取一块
  # 将训练集所有帧分为 num-archive 份，frames-per-iter 就是每个 archive 包含的"理想"帧数。
  # 从每个 utterance 截取一个chunk，allocate_egs.py 中的 this_num_egs 就是"理想"块数
  # 但 --num-repeats 比较小的时候达不到这个理想值。len_this_egs 是实际的块数
